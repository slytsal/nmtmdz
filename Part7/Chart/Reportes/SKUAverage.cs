...
namespace IMC.Wpf.CoverFlow.NMT.ChartComponent.Reportes
{
    ...
    /// Reporte
    /// </summary>
    class SKUAverage : IMC.Wpf.CoverFlow.NMT.ChartComponent.IChart
    {
        ...
        public List<Filtro> ObtenerFiltros()
        {
            ...
            DataTable filtro1 = consulta.SelectKPI_SKU_Average_Flt_SegmentPrice().Tables[0];
            Filtro filterSegmentPrice = Herramientas.Instance.obtenerFiltro(filtro1, "Segmento de Precio", "SegmentPrice", "SegmentPrice", false);
        ...
        public Chart Draw(params string[] filtros) 
        {
            ...
            try
            {
                kpis = new DataTable();
                Consultas consulta = new Consultas();
                kpis = consulta.SelectKPI_SKU_Average(filtros).Tables[0];
                consulta = null;
                string serAnt = "-5555";
                string ser = "-1111";
                foreach (DataRow g in kpis.Rows)
                {
                    Decimal? y = g["SKUAvg"]!=DBNull.Value?(Decimal?)g["SKUAvg"]:null;
