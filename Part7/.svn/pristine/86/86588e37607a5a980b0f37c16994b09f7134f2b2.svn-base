...
namespace IMC.Wpf.CoverFlow.NMT.ChartComponent.Reportes
{
    ...
    /// Reporte
    /// </summary>
    class AgotamientoNumerico : IMC.Wpf.CoverFlow.NMT.ChartComponent.IChart
    {
       ...
       /// <returns></returns>
        ...
        public Chart Draw(params string[] filtros) 
        {
            ...
            // Set title property
            title.Text = "Agotamiento Numerico";
            ...
            try
            {
                kpis = new DataTable();
                Consultas consulta = new Consultas();
                kpis = consulta.SelectKPI_Agotamiento(filtros).Tables[0];
                consulta = null;
                string serAnt = "-5555";
                string ser = "-1111";
                foreach (DataRow g in kpis.Rows)
                {
                    Decimal? y =g["NbrOOS"]!=DBNull.Value? (Decimal?)g["NbrOOS"]:null;
