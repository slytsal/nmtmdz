...
namespace IMC.Wpf.CoverFlow.NMT.ChartComponent.Reportes
{
    ...
    /// Reporte
    /// </summary>
    class MDZGeneralDropSize : IMC.Wpf.CoverFlow.NMT.ChartComponent.IChart
    {
        ...
        public Chart Draw(params string[] filtros) 
        {
            ...
            // Set title property
            title.Text = "MDZ General Drop Size";
            ...
            try
            {
                kpis = new DataTable();
                Consultas consulta = new Consultas();
                kpis = consulta.SelectKPI_MDZ_GENERAL_DROP_SIZE(filtros).Tables[0];
                consulta = null;
                string serAnt = "-5555";
                string ser = "-1111";
                foreach (DataRow g in kpis.Rows)
                {
                    Decimal? y = g["GeneralDropSize"]!=DBNull.Value?(Decimal?)g["GeneralDropSize"]:null;
