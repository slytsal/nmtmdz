...
namespace IMC.Wpf.CoverFlow.NMT.ChartComponent.Reportes
{
    ...
    /// Reporte
    /// </summary>
    class MDZEffectiveDropSize : IMC.Wpf.CoverFlow.NMT.ChartComponent.IChart
    {
        ...
        public List<Filtro> ObtenerFiltros()
        {
            ...
            DataTable filtro1 = consulta.SelectKPI_MDZ_EFFECTIVE_DROP_SIZE_Flt_IndustryLevel().Tables[0];
        ...
        public Chart Draw(params string[] filtros) 
        {
            ...
            // Set title property
            title.Text = "MDZ Effective Drop Size";
            ...
            try
            {
                kpis = new DataTable();
                Consultas consulta = new Consultas();
                kpis = consulta.SelectKPI_MDZ_EFFECTIVE_DROP_SIZE(filtros).Tables[0];
                consulta = null;
                string serAnt = "-5555";
                string ser = "-1111";
                foreach (DataRow g in kpis.Rows)
                {
                    Decimal? y = g["EffectiveDropSize"]!=DBNull.Value?(Decimal?)g["EffectiveDropSize"]:null;
