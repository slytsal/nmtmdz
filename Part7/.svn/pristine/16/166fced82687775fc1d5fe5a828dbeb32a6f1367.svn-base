...
namespace IMC.Wpf.CoverFlow.NMT.ChartComponent.Reportes
{
    ...
    /// Reporte
    /// </summary>
    class SobreIndexados : IMC.Wpf.CoverFlow.NMT.ChartComponent.IChart
    {
        ...
        public List<Filtro> ObtenerFiltros()
        {
            ...
            DataTable filtro1 = consulta.SelectKPI_Sobreindexados_Flt_IndustryLevel().Tables[0];
            Filtro filterIndustryLevel = Herramientas.Instance.obtenerFiltro(filtro1, "Potencial", "IndustryLevel", "IndustryLevel", false);
            DataTable filtro2 = consulta.SelectKPI_Sobreindexados_Flt_ProductSubfamilyCode().Tables[0];
            Filtro filterProductSubfamilyCode = Herramientas.Instance.obtenerFiltro(filtro2, "Product Subfamily Code", "ProductSubfamilyCode", "ProductSubfamilyCode", true);
            DataTable filtro3 = consulta.SelectKPI_Sobreindexados_Flt_ProductSubfamilyDescription().Tables[0];
            Filtro filterProductSubfamilyDescription = Herramientas.Instance.obtenerFiltro(filtro3, "Marca", "ProductSubfamilyDescription", "ProductSubfamilyDescription", true);//Subfamilia
        ...
        public Chart Draw(params string[] filtros) 
        {
            ...
            try
            {
                kpis = new DataTable();
                Consultas consulta = new Consultas();
                kpis = consulta.SelectKPI_Sobreindexados(filtros).Tables[0];
                consulta = null;
                string serAnt = "-5555";
                string ser = "-1111";
                foreach (DataRow g in kpis.Rows)
                {
                    int? y =g["CustomerCode"]!=DBNull.Value? (int?)g["CustomerCode"]:null;
